    ...
    def get_fingerprints(self, df, model, fp_name, split, numpy_folder):
        ...
        for smi in smiles_list:
            try: 
                ...
                descriptor_values = []
                for descriptor_name in specific_descriptors:
                    descriptor_func = getattr(rdMolDescriptors, descriptor_name, None)
                ...
                # Convert descriptor_values to a NumPy array before appending
                self.fingerprints.append(np.array(descriptor_values))
            except:
                # Handle exceptions and add placeholder values
                self.fingerprints.append(np.array([np.nan] * len(specific_descriptors)))
